...
def tune_probes_on_layer(X: np.ndarray, y: np.ndarray, random_state: int = 42)-> Dict[str, Dict[str, Any]]:
    """
    ...
    linsvm_pipe = Pipeline([
        ("sc", StandardScaler(with_mean=True, with_std=True)),
        ("clf", LinearSVC(class_weight="balanced", max_iter=10000, dual=(n < d)))
    ])
    linsvm_grid = {
        "clf__C": [0.1, 0.5, 1.0, 2.0, 5.0],
        "clf__tol": [1e-3, 1e-4]
    }
    linsvm_cv = GridSearchCV(linsvm_pipe, linsvm_grid, cv=cv, scoring=scoring, n_jobs=-1, refit=True)
    linsvm_cv.fit(X, y)
...
def fit_eval_probes_per_layer(
    ...
    X_test_layers: Dict[int, np.ndarray],  y_test: np.ndarray,
    regularize_lda: float = 1e-3,best_params: Dict[str, Dict[str, Any]] | None = None
) -> Dict[str, Dict[int, dict]]:
    
    results = {'lda': {}, 'logreg': {}, 'linsvm': {}}
    ...
    for li, Xtr in X_train_layers.items():
        ...
        try:
            ...
            results['lda'][li] = {'acc': cm0['acc'], 'auroc': au, 'fisher': fs,
                                  ...
                                  'cm_thr0': cm0, 'cm_thr_star': cm_star}
